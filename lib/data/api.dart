...
class Api {
  ...
  static Future<Instructor> me(String token) async {
    final response =
        await http.get(API_URL + '/instructor/me', headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
  ...
  static Future<List<Lesson>> lessons(String token) async {
    final response = await http
        .get(API_URL + '/instructor/lessons', headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
      'Authorization': 'Bearer ' + token,
    });
    if (response.statusCode == 200) {
  ...
  static Future<void> firebaseToken(String token, String firebaseToken) async {
    final response = await http.post(API_URL + '/instructor/devices',
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer ' + token,
        },
